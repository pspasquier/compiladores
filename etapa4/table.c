...
SymbolEntry* findFirstDeclaration(char* id, int line, Nature nature_used) {
    if (stack != NULL && stack->top != NULL) {
        NodeS* current = stack->top;
        while (current != NULL) {
            ...
            while (listNode != NULL) {
                ...
                if (strcmp(listNode->entry.id, id) == 0) {
                    if (listNode->entry.nature != nature_used) {
                        if (listNode->entry.nature == IDENTIFICADOR) {
                            printf("entrada:%d: error: identifier '%s' was used wrongly as a function (ERR_VARIABLE)\n", line, id);
                        } 
                        else {
                            printf("entrada:%d: error: function '%s' was used wrongly as a indentifier (ERR_FUNCTION)\n", line, id);
                        }
                        deleteStack();
                        exit(ERR_FUNCTION);
                    }
...
void addEntryToScope(const char* id, int linha, Nature nature, E_Type e_type) {
    ...
    if (!isAlreadyDeclared(id)) {
        ...
        printf("entrada:%d: error: identifier '%s' has already been declared in the local scope (ERR_DECLARED)\n", linha, id);
        deleteStack(); // Delete the entire stack and free memory before exiting
        exit(ERR_DECLARED);
    }
    printStack();
