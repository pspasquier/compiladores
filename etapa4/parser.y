...
program
    : { arvore = NULL; }
    | {stack = createStack(); push();} global_declarations { arvore = $<tree>1; pop();}
    ;
...
function_declaration
    : function_identifier new_scope '(' l_params ')' TK_OC_MAP type '{' l_commands '}' {
        $<tree>$ = ast_new($<lexical_value>1);
        ast_add_child($<tree>$, $<tree>7);
        pop();
    }
    | function_identifier new_scope '(' ')' TK_OC_MAP type '{' l_commands '}' {
        $<tree>$ = ast_new($<lexical_value>1);
...
function_identifier
    :  TK_IDENTIFICADOR {addEntryToScope($<lexical_value>1.label, $<lexical_value>1.line, FUNCAO, INT);}
    ;

l_params
    : type TK_IDENTIFICADOR {{addEntryToScope($<lexical_value>2.label, $<lexical_value>2.line, IDENTIFICADOR, INT);}}
    | l_params ',' type TK_IDENTIFICADOR {{addEntryToScope($<lexical_value>4.label, $<lexical_value>4.line, IDENTIFICADOR, INT);}}
    ;
...
l_global_variables
    : TK_IDENTIFICADOR {{addEntryToScope($<lexical_value>1.label, $<lexical_value>1.line, IDENTIFICADOR, INT);}}
    | l_global_variables ',' TK_IDENTIFICADOR {{addEntryToScope($<lexical_value>3.label, $<lexical_value>3.line, IDENTIFICADOR, INT);}}
    ;

command_block
    : '{' new_scope l_commands '}' { $<tree>$ = $<tree>2; pop();}
    | '{' '}' { $<tree>$ = NULL; }
    ;
...
l_local_variables
    : TK_IDENTIFICADOR { $<tree>$ = NULL; {addEntryToScope($<lexical_value>1.label, $<lexical_value>1.line, IDENTIFICADOR, INT);}}
    ...
    | l_local_variables ',' TK_IDENTIFICADOR  { $<tree>$ = $<tree>1; addEntryToScope($<lexical_value>3.label, $<lexical_value>3.line, IDENTIFICADOR, INT);}
    | l_local_variables ',' TK_IDENTIFICADOR TK_OC_LE literal {
	$<tree>$ = $<tree>1; addEntryToScope($<lexical_value>3.label, $<lexical_value>3.line, IDENTIFICADOR, INT);
        if ($<tree>1 == NULL) {
...
attribuition
	...
	findFirstDeclaration($<lexical_value>1.label, $<lexical_value>1.line, IDENTIFICADOR);
...
function_call
	...
	findFirstDeclaration($<lexical_value>1.label, $<lexical_value>1.line, FUNCAO);
        $<tree>$ = ast_new($<lexical_value>1);
        ast_add_child($<tree>$, $<tree>3);
...
operand
    ...
    | TK_IDENTIFICADOR { $<tree>$ = ast_new($<lexical_value>1); findFirstDeclaration($<lexical_value>1.label, $<lexical_value>1.line, IDENTIFICADOR);}
    | literal { $<tree>$ = $<tree>1; }
    | function_call { $<tree>$ = $<tree>1; }
    ;
